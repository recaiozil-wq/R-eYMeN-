...
Resolution order:
  1. ``ReYMeN_TIMEZONE`` environment variable
    ...
    try:
        ...
        if config_path.exists():
            ...
            if isinstance(tz_cfg, str) and tz_cfg.strip():
                return tz_cfg.strip()
    except Exception as _ReYMeN_t_e57:
        print(f"[UYARI] ReYMeN_time.py:58 - {_ReYMeN_t_e57}")
...
def reset_cache() -> None:
    """Force re-resolution of cached timezone on the next ``get_timezone()`` call.

    Call after config changes (e.g. user edits ``config.yaml`` at runtime).
    """
    global _cached_tz, _cached_tz_name, _cache_resolved  # noqa: PLW0603
    _cached_tz = None
