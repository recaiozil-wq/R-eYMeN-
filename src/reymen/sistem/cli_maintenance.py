...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    wt_name = f"ReYMeN-{short_id}"
    branch_name = f"ReYMeN/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`ReYMeN_cli.config.load_config` (the authoritative loader that
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_ReYMeN_home_maint / "sessions"
                )
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``ReYMeN/ReYMeN-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``ReYMeN -w`` sessions and PR review
