    ...
    if loop and loop.is_running():
        ...
        def _run_in_worker():
            ...
            try:
                ...
                except Exception as _model_to_e137:
                    print(f"[UYARI] model_tools.py:138 - {_model_to_e137}")
                worker_loop.close()
        ...
        try:
            ...
            if loop_ready.wait(timeout=1.0) and worker_loop is not None:
                try:
                    ...
                    logger.warning("[ModelTools] except RuntimeError (L153): %s", RuntimeError)
                    pass
            raise
        finally:
...
#   - cli.py, reymen_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like reymen-cli)
...
def _tool_result_observer_fields(result: Any) -> tuple[str, Optional[str], Optional[str]]:
    ...
    except Exception as _model_to_e819:
        print(f"[UYARI] model_tools.py:820 - {_model_to_e819}")
    return "ok", None, None
    ...
    try:
        ...
        try:
            ...
            if _approval_tokens is not None and reset_current_observability_context is not None:
                ...
                except Exception as _model_to_e1145:
                    print(f"[UYARI] model_tools.py:1146 - {_model_to_e1145}")
