...
_UNSET = object()
_ReYMeN_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ReYMeN_HOME_OVERRIDE", default=_UNSET
)


def set_ReYMeN_home_override(path: str | Path | None) -> Token:
    """Set a context-local ReYMeN home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ReYMeN_HOME_OVERRIDE.set(value)


def reset_ReYMeN_home_override(token: Token) -> None:
    """Restore the previous context-local ReYMeN home override."""
...
def get_reymen_home() -> Path:
    ...
    When ``ReYMeN_HOME`` is unset but an ``active_profile`` file indicates
    a non-default profile is active, logs a loud one-shot warning to
    ``errors.log`` so cross-profile data corruption is diagnosable instead
    of silent.  Behavior is unchanged otherwise — we still return
    ``~/.ReYMeN`` — because raising here would brick 30+ module-level
    callers that import this at load time.  Subprocess spawners are
    expected to propagate ``ReYMeN_HOME`` explicitly (see the systemd
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                ...
                f"subprocess spawner should pass ReYMeN_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
                sys.stderr.write(msg + "\n")
                sys.stderr.flush()
            except Exception as _ReYMeN_c_e97:
                print(f"[UYARI] ReYMeN_constants.py:98 - {_ReYMeN_c_e97}")
...
def get_default_reymen_root() -> Path:
    ...
    In Docker or custom deployments where ``ReYMeN_HOME`` points outside
    ``~/.ReYMeN`` (e.g. ``/opt/data``), returns ``ReYMeN_HOME`` directly
    — that IS the root.

    In profile mode where ``ReYMeN_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.ReYMeN/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = Path.home() / ".ReYMeN"
    ...
    try:
        ...
        # ReYMeN_HOME is under ~/.ReYMeN (normal or profile mode)
        return native_home
    except ValueError as _ReYMeN_c_e128:
        print(f"[UYARI] ReYMeN_constants.py:129 - {_ReYMeN_c_e128}")
    ...
    # Not a profile path — ReYMeN_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when ReYMeN is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ReYMeN_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ReYMeN_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``ReYMeN_OPTIONAL_MCPS``.
    """
    override = os.getenv("ReYMeN_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ReYMeN_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ReYMeN_HOME>/skills`` last-resort
    """
    override = os.getenv("ReYMeN_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_reymen_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to ReYMeN_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to ReYMeN_HOME (e.g. ``"image_cache"``).
...
def display_reymen_home() -> str:
    ...
    Uses ``~/`` shorthand for readability::
        ...
        default:  ``~/.ReYMeN``
        profile:  ``~/.ReYMeN/profiles/coder``
        custom:   ``/opt/ReYMeN-custom``
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``ReYMeN_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/ReYMeN-agent/issues/25821.
    """
    ...
    except OSError as _ReYMeN_c_e272:
        print(f"[UYARI] ReYMeN_constants.py:273 - {_ReYMeN_c_e272}")


def get_subprocess_home() -> str | None:
    """Return a per-profile HOME directory for subprocesses, or None.

    When ``{ReYMeN_HOME}/home/`` exists on disk, subprocesses should use it
    as ``HOME`` so system tools (git, ssh, gh, npm …) write their configs
    inside the ReYMeN data directory instead of the OS-level ``/root`` or
    ``~/``.  This provides:
    ...
    try:
        with open("/proc/1/cgroup", "r", encoding="utf-8") as f:
            ...
            if "docker" in cgroup or "podman" in cgroup or "/lxc/" in cgroup:
                _container_detected = True
                return True
    except OSError as _ReYMeN_c_e379:
        print(f"[UYARI] ReYMeN_constants.py:380 - {_ReYMeN_c_e379}")
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_ReYMeN_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._ReYMeN_ipv4_patched = True  # type: ignore[attr-defined]
