    ...
    *,
    ...
    try:
        ...
        if current_mode and current_mode[0] == "wal":
            return "wal"
    except sqlite3.OperationalError as _ReYMeN_s_e187:
        print(f"[UYARI] ReYMeN_state.py:188 - {_ReYMeN_s_e187}")
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``ReYMeN_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. This usually means ReYMeN is running on an "
            ...
            "https://ReYMeN-agent.nousresearch.com (underlying error: %s)",
            self.db_path,
            exc,
        )
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._ReYMeN_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._ReYMeN_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _drop_fts_triggers(cursor: sqlite3.Cursor) -> None:
        for trigger in _FTS_TRIGGERS:
            ...
            except sqlite3.OperationalError as _ReYMeN_s_e480:
                print(f"[UYARI] ReYMeN_state.py:481 - {_ReYMeN_s_e480}")
    ...
    def close(self):
        ...
        with self._lock:
            if self._conn:
                ...
                except Exception as _ReYMeN_s_e630:
                    print(f"[UYARI] ReYMeN_state.py:631 - {_ReYMeN_s_e630}")
                self._conn.close()
                self._conn = None
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 12:
                ...
                except sqlite3.OperationalError as _ReYMeN_s_e872:
                    print(f"[UYARI] ReYMeN_state.py:873 - {_ReYMeN_s_e872}")
    ...
    def set_session_archived(self, session_id: str, archived: bool) -> bool:
        """Set the archived flag on a session and its compression-parent chain.

        When archiving a compression tip, the projected root is also archived
        so the entire compression chain stays hidden from the default view.
    ...
    def list_sessions_rich(
        ...
        Pass ``archived_only=True`` to return only archived sessions (default
        hides archived sessions from the result).
        """
        where_clauses = []
        params = []
    ...
    def _remove_session_files(sessions_dir: Optional[Path], session_id: str) -> None:
        ...
        for suffix in (".json", ".jsonl"):
            ...
            except OSError as _ReYMeN_s_e3157:
                print(f"[UYARI] ReYMeN_state.py:3158 - {_ReYMeN_s_e3157}")
        # request_dump files use session_id as a prefix component
        try:
            for p in sessions_dir.glob(f"request_dump_{session_id}_*.json"):
                try:
                    p.unlink(missing_ok=True)
                except OSError as _ReYMeN_s_e3164:
                    print(f"[UYARI] ReYMeN_state.py:3165 - {_ReYMeN_s_e3164}")
        except OSError as _ReYMeN_s_e3166:
            print(f"[UYARI] ReYMeN_state.py:3167 - {_ReYMeN_s_e3166}")
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade ReYMeN, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one ReYMeN session.

        A ReYMeN session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a ReYMeN session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ReYMeN_DISABLE_FTS_TRIGRAM`` or not yet created), so
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            except Exception as _ReYMeN_s_e3794:
                print(f"[UYARI] ReYMeN_state.py:3795 - {_ReYMeN_s_e3794}")
